...
bool real(reader_ptr &reader, std::string &s) {
    ...
    const static auto *digits = &digits_dec_many1;
    if (one('0')(reader, c)) {
        s.push_back(c);
        if (base_list(reader, c)) {
            s.push_back(c);

            switch (base_number(c).value()) {
            case 2:
                digits = &digits_bin_many1;
                break;
            case 4:
                digits = &digits_quad_many1;
                break;
            case 8:
                digits = &digits_oct_many1;
                break;
            case 10:
                digits = &digits_dec_many1;
                break;
            case 16:
                digits = &digits_hex_many1;
                break;
            }
        }
    }
    ...
    const static auto dot_e = list(".eE");
    if (!dot_e(reader, c)) {
        return false;
    }
    s.push_back(c);
    ...
    const static auto digit_many1 = many1(digit_dec);
    return digit_many1(reader, s);
}
